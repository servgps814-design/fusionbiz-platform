...
export function MediaPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { publicUrl } = await blink.storage.upload(file, `media/${company.id}/${Date.now()}.${ext}`);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        ...
        <label className={cn('cursor-pointer', uploading && 'pointer-events-none opacity-50')}>
          ...
          <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold cursor-pointer bg-primary text-primary-foreground hover:bg-primary/90 transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
